...
class JCGTrainer(Trainer):

    def __init__(self, config, model):
        super(JCGTrainer, self).__init__(config, model)
    ...
    def fit(self, train_data, valid_data=None, verbose=True, saved=True, show_progress=False, callback_fn=None):
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            # the K-MEANS clustering depends on the semantic CL task
            if epoch_idx % self.num_m_step == 0:
                self.logger.info("Running k_means clustering! ")
                # self.model.update_INP_kmeans()
                self.model.update_COP_kmeans()
                # self.model.update_HYP_kmeans()
